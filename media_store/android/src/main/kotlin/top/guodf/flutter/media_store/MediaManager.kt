...
class MediaManager(private val flutterPluginBinding: FlutterPlugin.FlutterPluginBinding) {
    ...
    fun getAllMediaList(): List<MediaInfo> {
        val contentResolver = flutterPluginBinding.applicationContext.contentResolver
        ...
        arrayOf(Image, Video).forEach { mediaType ->
            val mediaId =
                if (mediaType == Image) MediaStore.Images.Thumbnails.IMAGE_ID else MediaStore.Video.Thumbnails.VIDEO_ID
...
//        }
    ...
    private fun mediaQuery(
        ...
        val cursor = contentResolver.query(mediaUri, columns, where, null, sortOrder) ?: return
    ...
    fun getCacheDir(): String {
        return flutterPluginBinding.applicationContext.externalCacheDir?.absolutePath
            ?: flutterPluginBinding.applicationContext.cacheDir.absolutePath
    }
    ...
    fun createVideoThumbnail(videoPath: String, thumPath: String, result: MethodChannel.Result) {
        ...
        try {
            var bitmap = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                var size = android.util.Size(0, 100)
                ThumbnailUtils.createVideoThumbnail(File(videoPath), size, null)
            } else {
